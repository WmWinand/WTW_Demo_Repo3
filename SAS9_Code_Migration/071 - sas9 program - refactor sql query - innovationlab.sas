...
%let sasDataPath = /innovationlab-export/innovationlab/homes/T.Winand@sas.com/Data/SAS9_Migration_Data;
%let localDataPath = /innovationlab-export/innovationlab/homes/T.Winand@sas.com/Data/CSV_Files;
*****************************************************************************************;
...
* Create caslib to &sasDataPath and data *;
/* caslib casdata path="&sasDataPath"  */
...
proc casutil;
    load file="&sasDataPath/orders_demo.sas7bdat"
    casout='orders_demo'
    outcaslib='casuser'
    replace;

    load file="&localDataPath/DISCOUNT_LOOKUP.csv"
...
proc print data=casuser.orders_demo (obs=5);
run;

proc print data=casuser.discount_lookup (obs=5);
run;
********************************************************************************************;
...
proc cas;
  table.alterTable / caslib="casuser", name="orders_demo",
    columns={
...
proc freqtab data=casuser.orders_demo;
    tables Product Country DiscountCode Return OrderDate;
run;
********************************************************************************************;
...
proc mdsummary data=casuser.orders_demo;
    output out=casuser.orders_mdsstats (replace=yes);
run;

proc print data=casuser.orders_mdsstats;
run;
********************************************************************************************;
...
data casuser.orders_demo_calc_columns;
    set casuser.orders_demo;
    Year = year(OrderDate);
    Month = Month(OrderDate);
    TotalCost = Quantity * Cost;
    TotalPrice = Quantity * Price;    
...
create table o.orders_demo_final as
    select f.*, 
           l.pct_discount * .01 as pctDiscount, 
           l.discount_description
        from casuser.orders_demo_calc_columns as f left join 
             casuser.discount_lookup as l
        on f.DiscountCode = l.discountCode;
quit;

* Join the orders_demo_calc_columns and the Discount_Lookup tables using PROC FedSQL - CAS Tables*;
proc FedSQL sessref=mySession;
create table casuser.orders_demo_final as
    select f.*, 
           l.pct_discount * .01 as pctDiscount, 
           l.discount_description
        from casuser.orders_demo_calc_columns as f left join 
...
proc print data=casuser.orders_demo_final(obs=10);
run;
********************************************************************************************;
...
*********************************************************************************************;
proc mdsummary data=casuser.orders_demo_final;
  var TotalCost TotalPrice Profit;
  groupby / out=casuser.orders_summary;
  groupby Country Year /out=casuser.OrdersByCountryYear;
run;

ods graphics / reset width=6.4in height=4.8in imagemap;
proc sgplot data=CASUSER.ORDERSBYCOUNTRYYEAR;
